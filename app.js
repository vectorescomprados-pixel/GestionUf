document.addEventListener("DOMContentLoaded", function() {
    ...
    const handleSearch = () => {
        ...
        if(!t) { renderList(departamentos); addLog("Lista completa"); return; }
    ...
    window.renderList = (items) => {
        ...
        l.innerHTML = items.length ? "" : "<p style='text-align:center; padding:20px; opacity:0.5'>Sin datos</p>";
        items.forEach(d => {
            ...
            div.innerHTML = `
                <div class="item-header"><strong>UF: ${d.UF}</strong>
                    <div><button class="btn primary" onclick="editDept(${idx})">Editar</button>
                    <button class="btn danger" onclick="deleteDept(${idx})">X</button></div>
                </div>
    ...
    document.getElementById("btn-save").onclick = () => {
        ...
        if(editIndex === -1) {
            if(departamentos.some(x => x.UF === uf)) return alert("Existe");
    ...
    window.deleteDept = (i) => { if(confirm("¿Eliminar?")) { addLog(`Borró UF: ${departamentos[i].UF}`); departamentos.splice(i,1); saveData(); renderList(departamentos); } };
    ...
    document.getElementById("btn-export").onclick = () => {
        ...
        a.download = "BaseUF_Excel.csv"; a.click(); addLog("Exportó a Excel");
    };
    ...
    document.getElementById("csv-file").onchange = (e) => {
        ...
        reader.onload = (ev) => {
            ...
            for(let i=1; i<lines.length; i++){
                ...
                if(c.length >= 5 && !departamentos.some(x => x.UF === c[0].trim())){
                    departamentos.push({UF:c[0].trim(), Propietario:c[1], TelefonoPropietario:c[2], Inquilino:c[3], TelefonoInquilino:c[4]});
                }
            }
    ...
    const init = () => {
        ...
        const ent = (e) => { if(e.key === "Enter") handleLogin(); };
        document.getElementById("login-user").onkeypress = ent;
        document.getElementById("login-pass").onkeypress = ent;
        ...
        document.getElementById("btn-clear").onclick = () => { document.getElementById("search-text").value = ""; document.getElementById("list").innerHTML = ""; addLog("Limpió"); };
        ...
        document.getElementById("btn-verify-phone").onclick = () => {
            ...
            if (!st2.classList.contains("hidden")) {
                users.find(u => u.username === "admin").password = document.getElementById("new-admin-pass").value.trim();
                saveUsers(); alert("Listo"); location.reload();
            } else if(ph === masterPhone) { st2.classList.remove("hidden"); document.getElementById("btn-verify-phone").innerText = "Guardar"; }
        };
        document.getElementById("btn-delete-db").onclick = () => { if(prompt("Pass Admin:") === currentUser.password) { departamentos = []; saveData(); renderList([]); addLog("BORRÓ TODO"); } };
